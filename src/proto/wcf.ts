...
export namespace wcf {
    ...
    export class MsgType extends pb_1.Message {
        #one_of_decls: number[][] = [];
        constructor(data?: any[] | {
            code?: number;
            label?: string;
        ...
        static deserialize(bytes: Uint8Array | pb_1.BinaryReader): MsgType {
            const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new MsgType();
            while (reader.nextField()) {
                if (reader.isEndGroup())
                    break;
                switch (reader.getFieldNumber()) {
        ...
        static deserializeBinary(bytes: Uint8Array): MsgType {
            return MsgType.deserialize(bytes);
        }
    }
    export class MsgTypes extends pb_1.Message {
        ...
        get types() {
            return pb_1.Message.getRepeatedWrapperField(this, MsgType, 1) as MsgType[];
        }
        set types(value: MsgType[]) {
            pb_1.Message.setRepeatedWrapperField(this, 1, value);
        }
        static fromObject(data: {
            types?: ReturnType<typeof MsgType.prototype.toObject>[];
        }): MsgTypes {
            const message = new MsgTypes({});
            if (data.types != null) {
                message.types = data.types.map(item => MsgType.fromObject(item));
            }
            return message;
        }
        toObject() {
            ...
            if (this.types != null) {
                data.types = this.types.map((item: MsgType) => item.toObject());
            }
            return data;
        }
        serialize(): Uint8Array;
        ...
        serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
            ...
            if (this.types.length)
                writer.writeRepeatedMessage(1, this.types, (item: MsgType) => item.serialize(writer));
            if (!w)
                return writer.getResultBuffer();
        }
        static deserialize(bytes: Uint8Array | pb_1.BinaryReader): MsgTypes {
            ...
            while (reader.nextField()) {
                ...
                switch (reader.getFieldNumber()) {
                    case 1:
                        reader.readMessage(message.types, () => pb_1.Message.addToRepeatedWrapperField(message, 1, MsgType.deserialize(reader), MsgType));
                        break;
                    default: reader.skipField();
                }
            }
